...
def smart_load_model(
    ...
    if not os.path.exists(model_path):
        ...
        try:
            ...
            # Only download the specified subdirectory
            path = snapshot_download(
                repo_id=original_model_path,
                # allow_patterns=[f"{subfolder}/*"],  # Key change: pattern match subfolder
                local_dir=model_path,
            )
            model_path = path  # os.path.join(path, subfolder)  # Keep the original path join logic
        except ImportError:
            logger.warning(
